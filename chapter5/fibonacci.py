...
class THINK_NODES(ActionNode):
    def __init__(
        ...
        super().__init__(
            key=name, expected_type=str, instruction=instruction, example=example
        )
        self.add_children(
    ...
    async def fill(self, context, llm, schema="raw", mode="auto", strgy="complex"):
        ...
        if strgy == "simple":
            return await self.simple_fill(to=schema, mode=mode)
        elif strgy == "complex":
            # 这里隐式假设了拥有children
...
class Printer(Role):
    ...
    async def _act(self) -> Message:
        ...
        if type(todo) is ThinkAction:
            ...
            return await self._prepare_print(
                resp
            )  # 执行完ThinkAction后，根据返回的列表，设置SimplePrint
